...
%for iter_a=1:2500
while(1)
    J(iter+1)=J_Cost(x,y,w1(iter));
    [Derivate_W1(iter)]=Gradient_Descent(x,y,w1(iter));
...
function error_value=J_Cost(x,y,omega1)
    m=length(x);
    sumarray=(((omega1*x)-y).^2);
    innervalue=sum(sumarray);
    error_value=1/(2*m)*innervalue;
end
function [error_value_w1,error_value_w0]=Gradient_Descent(x,y,omega1)
    m=length(x);
    innervalue=2*((omega1*x)-y);
